...
import { getQuizPreview } from '../duck/network';
...
export default function QuizPreview() {
  ...
  const { villageAreaId, programUnitContentId, quizNumber, childId } = location.state || {};
  const QuizPreviewData = useSelector((state) => state.quiz.quizpreview);
  const loading = useSelector((state) => state.quiz.quizloading)

  useEffect(() => {
    dispatch(getQuizPreview({
      programUnitContentId: programUnitContentId, villageAreaId: villageAreaId, parentId: params.id,
  ...
  return (
    <>
      <Link onClick={Back} className='tw-mb-8'>
      ...
      {loading ?
        ...
        <>
          <div className='tw-flex tw-justify-between tw-w-full tw-items-center tw-mb-6'>
            <Typography variant="h2" className='!tw-font-semibold !tw-text-secondaryText'>{QuizPreviewData?.parentName}</Typography>
            <span className='tw-text-primaryText tw-text-lg'>Score: <strong className='tw-text-secondaryText'>{QuizPreviewData?.quizScore === null ? "-"  : QuizPreviewData?.quizScore}</strong></span>
          </div>
          <div className='tw-flex tw-flex-col gap-2 tw-mb-10'>
            <span className='tw-text-xs tw-text-grey tw-block tw-mb-1'>{QuizPreviewData?.quizUnit}</span>
            <span className='tw-text-grey tw-text-base'>{quizNumber}: {QuizPreviewData?.quizTitle}</span>
          </div>
          {QuizPreviewData?.submittedQuiz?.map((el, optionIndex) => (
            <div className={`${QuizPreviewData?.submittedQuiz.length > 1 ? "tw-pb-7 tw-mb-7 tw-border-b tw-border-[#CCC]" : ""}`} key={optionIndex}>
