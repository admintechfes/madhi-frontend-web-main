...
import { BulkUpload, BulkUploadList } from '../duck/network';
...
export default function ManageBulkUploadParent() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const paginateInfo = useSelector((state) => state.parents.parentsListAddedPaginateInfo)
  const [page, setPage] = useState(1);
  const [limitPerPage, setLimitPerPage] = useState(10)
  const [list, setList] = useState([])
  const bulkimportloading = useSelector((state) => state.parents.bulkimportLoading);
  ...
  const formatForDisplay = data => {
    ...
    data?.forEach((item, index) => {
      formatedRows.push({
        ...
        "role": item.uploader?.roles[0]?.name,
  ...
  const handleUpload = () => {
    ...
    dispatch(BulkUpload(formData)).then((res) => {
      setOpenAttachmentDialog(false)
      setKey(Math.random())
  ...
  const OnDownloadSampleTemplate = async () => {
    const url = `/v1/parents/bulk_upload/download-sample`;
    try {
      setIsLoading(true);
    ...
    <div>
      <div className="tw-flex tw-flex-col tw-w-full tw-mb-5 tw-gap-1">
        ...
        <div className="tw-flex tw-justify-between tw-px-3">
          <Typography variant="h3" className='!tw-font-semibold'>Manage Parents Bulk Upload</Typography>
      ...
      <FormDialog open={openAttachmentDialog} close={handleDialogClose} title="Add Attachments">
        <form onSubmit={handleSubmit(handleUpload)}>
          ...
          <div className="tw-pt-8 tw-pb-1 tw-flex tw-justify-end tw-gap-5">
            ...
            <div className="tw-grow">
              <LoadingButton loading={bulkimportloading} type='submit' className={`${selectedFile ? '' : '!tw-bg-[#0000001f]'}`} fullWidth variant="contained" disableElevation disabled={selectedFile ? false : true}>Add</LoadingButton>
            </div>
          </div>
        </form>
      </FormDialog>
