...
import { fillCampaingDetails, fillCampaingParentList, fillCampingList, fillGlificFlowNameList, fillTagsNameList, setLoadingCamping, setLoadingUser, fillPageNum, fillPerPageNum, fillWhatsAppStatus, fillWhatsAppStatusNew } from './manualWhatsAppSlice';

export const postManualWhatsAppCreate = (params) => async (dispatch) => {
	const url = '/v1/communications/campaigns/manual-whatsapp/create/campaign/v1';
	try {
...
export const getTagNameList = (params) => async (dispatch) => {
...
export const getManualWhatsAppStatusNew = (params) => async (dispatch) => {
	let url = `/v1/communications/campaigns/manual-whatsapp/get/master/status`

	try {
		dispatch(setLoadingUser(true));
		const response = await axiosInstance.post(url);
		dispatch(fillWhatsAppStatusNew(response?.data.data))
		dispatch(setLoadingUser(false));
		return response?.data?.data
	} catch (err) {
		toast.error(err.response?.data?.userMessageTitle)
