...
import { getShareUnitList, postSharedQuizCreate } from '../parents/duck/network';
import { fillPageNumSharedQuizParents, fillPerPageNumSharedQuizParents, setSelectedSharedQuizQuestion } from '../parents/duck/parentsSlice';
import ManualWhatsAppParentTable from '../ManualWhatsApp/ManualWhatsAppParentTable';
import SharedQuizParentTable from './table';
...
export default function SharedParents() {
  ...
  const selected = useSelector((state) => state.parents.selectedSharedQuizQuestion)
  ...
  const sharedquizpaginateInfo = useSelector(state => state.parents.sharedquizpaginateInfo)
  ...
  const handleSelected = (selectedData) => {
    ...
    dispatch(setSelectedSharedQuizQuestion(selectedData))
  };
  ...
  const submitData = () => {
    ...
    dispatch(postSharedQuizCreate(finalValues)).then((resp) => {
  ...
  const handleBackParents = () => {
    dispatch(setSelectedSharedQuizQuestion(addedParents))
    navigate(-1)
  }
    ...
    <Box>
      ...
      <Paper className='tw-w-full tw-bg-white tw-pt-6 tw-mt-6'>
        <div className='tw-flex tw-justify-between tw-relative tw-items-center tw-pr-4'>
          {sharedquizpaginateInfo.total && <div className='tw-flex tw-gap-4 tw-items-center tw-justify-between tw-w-[400px]'>
            <span className='tw-text-sm tw-ml-4'>Total {sharedquizpaginateInfo.total}</span>
            {isSelectAll && <div className='tw-bg-backgroundPrimary tw-p-2 tw-border tw-rounded tw-flex tw-justify-between tw-items-center tw-w-[77%]'>
              <span className='tw-text-sm tw-text-[#333333]'>{!isSelectedAll && "Select"} {sharedquizpaginateInfo.total} parents {isSelectedAll && "selected"}</span>
              {!isSelectedAll ?
          ...
          <div className={`tw-flex tw-gap-4 tw-justify-end ${sharedquizpaginateInfo.total ? "w-auto" : "tw-w-full"}`}>
            ...
            <ParentsFilter type={true} setIsSelectAll={setIsSelectAll} setIsSelectedAll={setIsSelectedAll}
            ...
            <>
              {list.length ? (
                ...
                <SharedQuizParentTable
                  paginate={sharedquizpaginateInfo}
