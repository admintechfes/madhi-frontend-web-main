...
} from '../../../duck/workshopContentSlice';
import { createObservationForm, updateObservationForm } from '../../../duck/network';
import { DeleteDialog } from '../../../../../components/Dialog';

export default function ObservationFormsCreate(props) {
	...
	const loading = useSelector((state) => state.workshopContent.quizCreateLoading);
	const questionnaireData = useSelector((state) => state.workshopContent.selectedQuestionnaire);
	...
	const workshopId = location?.state?.workshop?.id;
	...
	useEffect(() => {
		...
		if (props?.type == 'update') {
			if (access?.Programs?.program_unit?.ws?.update) setSavePermission(true);
		} else {
			if (access?.Programs?.program_unit?.ws?.create) setSavePermission(true);
		}
	...
	const payloadConstruct = (formValues, questionnaire, programUnitId, parentContentId) => {
		let payload = { programUnitId, parentContentId, ...formValues };
	...
	const handleFormSubmit = async (formValues) => {
		if (props?.type == 'update') {
			let dispatchFormValues = payloadConstruct(formValues, questionnaireData, unitId, workshopId);
			const res = await dispatch(updateObservationForm(props.defaultValues.id, dispatchFormValues));
			if (res?.statusCode == 200) navigate(`/unit-content/workshop-content/${workshopId}`, { state: { unit: unit, workshop: workshop, programId: programId } });
		} else {
			let dispatchFormValues = payloadConstruct(formValues, questionnaireData, unitId, workshopId);
			const res = await dispatch(createObservationForm(dispatchFormValues));
			if (res?.statusCode == 200) navigate(`/unit-content/workshop-content/${workshopId}`, { state: { unit: unit, workshop: workshop, programId: programId } });
		}
	};
				...
				<form onSubmit={handleSubmit(handleFormSubmit)}>
					<div className="tw-flex tw-flex-wrap tw-gap-y-3 tw-justify-between">
						<h1 className="tw-font-bold tw-text-[24px]">
							<Button
								variant="text"
								onClick={() => navigate(`/unit-content/workshop-content/${workshopId}`, { state: { unit: unit, workshop: workshop, programId: programId } })}
								className="!tw-bg-backgroundColor !tw-text-grey !tw-text-[12px]"
								disableRipple
								startIcon={<KeyboardBackspaceIcon />}
							>
								{`program / ${location?.state?.unit?.serialNumber} / ${location?.state?.workshop?.serialNumber}`}
							</Button>
							<h1 className="tw-px-2 tw-mt-[-4px]">{props?.type == 'update' ? 'Edit Observation Form Details' : 'Create New Observation Form'}</h1>
						</h1>
						{savePermission && (
							<div className="tw-flex tw-gap-x-5">
								<div className="tw-flex tw-gap-x-5">
									...
									<LoadingButton
										...
										variant="contained"
										size="small"
										className={`tw-h-[35px] ${programStatus == 'closed' ? '!tw-bg-[#9999]' : ''}`}
									>
										Save
									</LoadingButton>
								</div>
							...
							<Container maxWidth={false}>
								<div className="tw-py-6">
									<div className="tw-flex tw-justify-between tw-items-center">
										<h1 className="tw-text-secondaryText tw-font-semibold tw-text-[20px]">Observation Form Details</h1>
									</div>
