...
export default function SurveysPreview() {
	...
	useEffect(() => {
		dispatch(getUnitContentSurveyDetail(params.id || unitContent.id));
	}, []);

	return <>{!loading ? (<QuizForm questionnaireType="Survey" type="preview" defaultValues={surveyDetail} />) : <div className="tw-flex tw-h-[100vh] tw-items-center tw-justify-center">{<CircularProgress />}</div>}</>;
}
