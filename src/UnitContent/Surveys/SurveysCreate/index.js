...
import { createSurvey, updateSurvey } from '../../duck/network';
...
export default function SurveysCreate(props) {
	...
	useEffect(() => {
		...
		setValue('contentFor', surveyForm?.contentFor ?? null);
	...
	const handleFormSubmit = async (formValues) => {
		...
		if (valid) {
			...
			const res = await dispatch(updateSurvey(props.defaultValues.id, dispatchFormValues));
			...
			let dispatchFormValues = payloadConstruct(formValues, questionnaireData, unitId);
			const res = await dispatch(createSurvey(dispatchFormValues));
			if (res?.statusCode === 200)
				navigate(`/programs-unit/unit-content/${unitId}`, {
					state: { unit: location?.state?.unit, programId: location.state.programId, programDetails: programDetails },
				});
					...
					<form onSubmit={handleSubmit(handleFormSubmit)}>
						<div className="tw-flex tw-flex-wrap tw-gap-y-3 tw-justify-between">
							<h1 className="tw-font-bold tw-text-[24px]">
								...
								<h1 className="tw-px-2 tw-mt-[-4px]">{props?.type == 'update' ? 'Edit Survey Details' : 'Create New Survey'}</h1>
							</h1>
							{savePermission && (
								...
								<Container maxWidth={false}>
									<div className="tw-py-6">
										<div className="tw-flex tw-justify-between tw-items-center">
											<h1 className="tw-text-secondaryText tw-font-semibold tw-text-[20px]">Survey Form Details</h1>
										</div>
